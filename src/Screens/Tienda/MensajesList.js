    ...
    if (!notificaciones) {
        return (
            <View style={{ flex: 1, justifyContent: "center", alignItems: "center" }}>
                <Text style={{ fontSize: 18, fontWeight: "bold", color: "#25d366" }}>{mensaje}</Text>
            </View>
        )
    }
...
function Notificacion({ notificacion, navigation }) {
    ...
    return (
        <TouchableOpacity
            onPress={() => { navigation.navigate("contacto", { displayName, phoneNumber, photoURL, email }) }}
        >
            <View style={styles.container}>
                ...
                <View>
                    <Text style={{ fontWeight: "bold" }}>{displayName}
                        <Text style={{ fontWeight: "normal" }}>{" "}te ha enviado un mensaje para el producto de</Text>
